...
void SoftwareSerial::begin(long speed) {
   ...
   m_bitTime = System.getCpuFreqMHz()*1000000/speed;
}

long SoftwareSerial::baudRate() {
   return System.getCpuFreqMHz()*1000000/m_bitTime;
}
...
size_t SoftwareSerial::readBytes(char* buffer, size_t maxLen)
{
	...
	while( System.getCycleCount() - start < mTimeout );
	
	return received;
}
   

#define WAIT { while (System.getCycleCount()-start < wait); wait += m_bitTime; }
...
size_t SoftwareSerial::write(uint8_t b) {
   ...
   unsigned long start = System.getCycleCount();
    // Start bit;
