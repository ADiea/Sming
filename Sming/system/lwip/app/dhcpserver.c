...
static void ICACHE_FLASH_ATTR send_offer(struct dhcps_msg *m)
{
...
#if DHCPS_DEBUG
		LOG_I("udhcp: send_offer>>p->ref = %d\n", p->ref);
#endif
	    if(p != NULL){
	       
#if DHCPS_DEBUG
	        LOG_I("dhcps: send_offer>>pbuf_alloc succeed\n");
	        LOG_I("dhcps: send_offer>>p->tot_len = %d\n", p->tot_len);
	        LOG_I("dhcps: send_offer>>p->len = %d\n", p->len);
#endif
	        q = p;
	        while(q != NULL){
	            data = (u8_t *)q->payload;
	    ...
	    LOG_I("dhcps: send_offer>>udp_sendto result %x\n", SendOffer_err_t);
#else
...
static void ICACHE_FLASH_ATTR send_nak(struct dhcps_msg *m)
{
	    ...
	    p = pbuf_alloc(PBUF_TRANSPORT, sizeof(struct dhcps_msg), PBUF_RAM);
#if DHCPS_DEBUG
		LOG_I("udhcp: send_nak>>p->ref = %d\n", p->ref);
#endif
	    if(p != NULL){
	        
#if DHCPS_DEBUG
	        LOG_I("dhcps: send_nak>>pbuf_alloc succeed\n");
	        LOG_I("dhcps: send_nak>>p->tot_len = %d\n", p->tot_len);
	        LOG_I("dhcps: send_nak>>p->len = %d\n", p->len);
#endif
	        q = p;
	        while(q != NULL){
	            data = (u8_t *)q->payload;
        ...
        LOG_I("dhcps: send_nak>>udp_sendto result %x\n",SendNak_err_t);
#else
        udp_sendto( pcb_dhcps, p, &broadcast_dhcps, DHCPS_CLIENT_PORT );
#endif
 	    if(p->ref != 0){
...
static void ICACHE_FLASH_ATTR send_ack(struct dhcps_msg *m)
{
	    ...
	    p = pbuf_alloc(PBUF_TRANSPORT, sizeof(struct dhcps_msg), PBUF_RAM);
#if DHCPS_DEBUG
		LOG_I("udhcp: send_ack>>p->ref = %d\n", p->ref);
#endif
	    if(p != NULL){
	        
#if DHCPS_DEBUG
	        LOG_I("dhcps: send_ack>>pbuf_alloc succeed\n");
	        LOG_I("dhcps: send_ack>>p->tot_len = %d\n", p->tot_len);
	        LOG_I("dhcps: send_ack>>p->len = %d\n", p->len);
#endif
	        q = p;
	        while(q != NULL){
	            data = (u8_t *)q->payload;
		...
		LOG_I("dhcps: send_ack>>udp_sendto result %x\n",SendAck_err_t);
#else
		udp_sendto( pcb_dhcps, p, &broadcast_dhcps, DHCPS_CLIENT_PORT );
#endif
	    if(p->ref != 0){
...
//	                }
	                ...
	                LOG_I("dhcps: client_address.addr = %x\n", client_address.addr);
#endif
									...
									uint16_t port)
{
...
#if DHCPS_DEBUG
	    LOG_I("dhcps: handle_dhcp-> p->tot_len = %d\n", tlen);
	    LOG_I("dhcps: handle_dhcp-> p->len = %d\n", p->len);
#endif		
		...
		if(p->next != NULL) {
	        ...
	        LOG_I("dhcps: handle_dhcp-> p->next->tot_len = %d\n",p->next->tot_len);
	        LOG_I("dhcps: handle_dhcp-> p->next->len = %d\n",p->next->len);
#endif
